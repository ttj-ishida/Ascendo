...
- **TODO: Expo Web版でのログイン等の動作確認**: `src/lib/secure-store-adapter.ts`が`expo-secure-store`(モバイルのみ対応)に固定されているため、Web版は起動直後に`ExpoSecureStore.default.getValueWithKeyAsync is not a function`でクラッシュし、ログイン画面より先に進めない(実際の動作確認セッションで確認済み、2026-08-16)。対応するには`Platform.OS === 'web'`で分岐し、Web版では`localStorage`(またはCookie等)を使うストレージ実装を追加する必要がある。Ascendoはモバイルアプリとして設計されているため優先度は低いが、開発中の動作確認をブラウザで手軽に行いたい場合は着手する価値がある
